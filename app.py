...
# Pehle se variables define kar diye taaki code crash na ho
...
try:
    ...
    # 🔴 Purana kharab data delete karne ke liye
...
# =======================================================
# 2. CHAT API ROUTE (Speed & Format Optimized)
# =======================================================
...
def chat():
    ...
    try:
        ...
        # --- OPTIMIZED MEMORY: Sirf pichli 5 chats uthayenge speed ke liye ---
        formatted_contents = []
        if chats_collection is not None:
            try:
                # Last 5 chats uthao sorted by newest
                past_chats = chats_collection.find({"sessionId": session_id}).sort("timestamp", -1).limit(5)
                chats_list = list(past_chats)[::-1] # Reverse order for timeline
        ...
        system_prompt = (
            f"You are a smart AI Assistant. Keep responses neat, short, and helpful. "
            f"LIVE TIME CONTEXT: Time: {current_time}, Date: {current_date}, Day: {current_day}."
        )
