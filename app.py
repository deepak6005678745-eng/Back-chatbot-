...
# Pehle Render ke Environment Variable se uthayega, agar wahan nahi mila toh backup string use karega
# NOTE: <db_password> ko hata kar apna asli password Render dashboard me ya yahan daal dena.
BACKUP_MONGO_URI = "mongodb+srv://deepak6005678745_db_user:<db_password>@cluster0.yn1lfwq.mongodb.net/chatbot_db?retryWrites=true&w=majority&appName=Cluster0"
MONGO_URI = os.getenv("MONGO_URI", BACKUP_MONGO_URI)

if not MONGO_URI or "<db_password>" in MONGO_URI:
    print("WARNING: MONGO_URI set nahi hai ya password change nahi kiya. Database connect nahi ho payega!")
...
# =======================================================
# 2. CHAT API ROUTE (With Database Memory Sync)
# =======================================================
...
def chat():
    ...
    try:
        ...
        # --- MEMORY LOGIC: MongoDB se pichli 15 chats uthao ---
        formatted_contents = []
        if chats_collection is not None:
            try:
                past_chats = chats_collection.find({"sessionId": session_id}).sort("timestamp", 1).limit(15)
                for c in past_chats:
        ...
        # --- DATABASE ME SAVE KARNA (History Store) ---
        if chats_collection is not None:
            try:
                chat_log = {
