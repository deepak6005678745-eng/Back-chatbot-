...
# Global variables pehle hi define kar diye safe side ke liye
...
try:
    ...
    # Purana kharab data saaf karne ke liye (Sirf ek baar chalega)
    chats_collection.delete_many({}) 
    print("MongoDB se connection ekdum makkhan chal gaya aur data saaf ho gaya!")
...
# =======================================================
# 2. CHAT API ROUTE (Bulletproof Memory)
# =======================================================
...
def chat():
    ...
    try:
        ...
        if chats_collection is not None:
            try:
                # Pichli 5 chats uthao speed aur format dono sahi rakhne ke liye
                past_chats = chats_collection.find({"sessionId": session_id}).sort("timestamp", -1).limit(5)
                chats_list = list(past_chats)[::-1]
        ...
        # Naya user message jodo
        formatted_contents.append({"role": "user", "parts": [user_message]})

        # --- NATIVE TIME LOGIC (No external library needed) ---
        utc_now = datetime.utcnow()
        ist_now = utc_now + timedelta(hours=5, minutes=30) # India Time Zone Manual Offset
        ...
        system_prompt = (
            ...
            f"Always use this exact information if the user asks about time or date."
        )
...
# =======================================================
# 3. CHAT HISTORY ROUTE (Frontend sidebar)
# =======================================================
